...
export const DailyBands: React.FC<DailyBandsProps> = ({ 
    reservations, rooms, onAddConsumption, consumptions, products, onCloseBand, onDayClose, onToggleAbono, onUpdateConsumption, isClosingView = false
}) => {
  ...
  const getRoomStyle = (color: RoomColor) => {
    switch(color) {
      case RoomColor.BLUE: return 'border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.2)]';
      case RoomColor.GREEN: return 'border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.2)]';
      case RoomColor.VIOLET: return 'border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.2)]';
      case RoomColor.RED: return 'border-red-500 shadow-[0_0_10px_rgba(239,68,68,0.2)]';
    }
  };
    ...
    <div className="space-y-6">
      ...
      {todaysBands.length === 0 && closedBandsToday.length === 0 ? (
          ...
          todaysBands.map(res => {
            ...
            return (
                <div key={res.id} className={`bg-gray-900 border-l-4 p-4 rounded ${getRoomStyle(room.color)} relative`}>
                    ...
                    <div className="mb-4 bg-black/20 p-3 rounded border border-gray-800">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3 max-h-40 overflow-y-auto">
                            {products.filter(p => p.category === 'BAR' && p.id !== 'bar_generic' && p.id !== 'pago_deuda').map(p => (
                                <button 
                                    key={p.id}
                                    onClick={() => onAddConsumption(res.id, p)}
                                    className="px-2 py-1 bg-gray-800 border border-gray-600 rounded text-xs hover:bg-gray-700 hover:border-[#D2B48C] text-left"
                                >
