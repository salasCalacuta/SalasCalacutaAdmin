...
interface GeneralBarViewProps {
  products: Product[];
  onAddTransaction: (amount: number, method: 'CASH' | 'MERCADOPAGO' | 'DEBT', descriptionSuffix?: string) => void;
  onUpdateStock: (product: Product) => void;
}

export const GeneralBarView: React.FC<GeneralBarViewProps> = ({ products, onAddTransaction, onUpdateStock }) => {
  ...
  const handlePay = (method: 'CASH' | 'MERCADOPAGO' | 'DEBT') => {
      ...
      if (method === 'DEBT') {
          ...
          if (!debtor) return; // Cancel if no name
          suffix = ` (Deuda: ${debtor})`;
      }
      ...
      // Add Transaction
      onAddTransaction(total, method, suffix);
  ...
  return (
    <div className="space-y-6">
        <div className="bg-gray-800 p-4 rounded border border-[#D2B48C]/30 text-center">
            <h2 className="text-xl font-bold text-[#D2B48C]">VENTA DE BARRA (General)</h2>
        </div>
                    ...
                    {products.filter(p => p.category === 'BAR' && p.id !== 'bar_generic' && p.id !== 'pago_deuda').map(p => (
                        <button 
                            ...
                            className={`p-3 rounded border text-left text-sm flex flex-col justify-between h-20 transition-all
                                ${p.stock < 1 ? 'border-red-900 bg-red-900/10 opacity-50 cursor-not-allowed' : 'border-gray-700 bg-gray-900 hover:border-[#D2B48C]'}
                            `}
                            disabled={p.stock < 1}
                        >
                    ...
                    {cart.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-gray-900 p-2 rounded border border-gray-800">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => removeFromCart(item.product.id)} className="text-red-500 hover:text-white">x</button>
                            </div>
                        </div>
                    ))}
                </div>
