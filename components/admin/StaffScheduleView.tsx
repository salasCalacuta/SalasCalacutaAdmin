...
export const StaffScheduleView: React.FC<StaffScheduleViewProps> = ({ shifts, onAddShift, onRemoveShift, onUpdateShift }) => {
  const [staff, setStaff] = useState(STAFF_USERS[0]);
                  ...
                  <tbody className="divide-y divide-gray-800">
                      {STAFF_USERS.map(user => (
                          <tr key={user} className="hover:bg-white/5">
                              ...
                              <td className="p-3 text-center">{staffSummary[user].count}</td>
                              <td className="p-3 text-center text-[#D2B48C] font-bold">
                                  {staffSummary[user].totalHours.toFixed(2)} hs
                              </td>
                          </tr>
                      ))}
                  </tbody>
