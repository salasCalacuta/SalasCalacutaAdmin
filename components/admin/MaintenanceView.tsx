...
type ModalType = 'ENVIAR' | 'PRESUPUESTO' | 'RETIRO' | null;

export const MaintenanceView: React.FC<MaintenanceViewProps> = ({ items, onUpdateItem, onPayRepair }) => {
  const [activeModal, setActiveModal] = useState<ModalType>(null);
  const [selectedItem, setSelectedItem] = useState<MaintenanceItem | null>(null);

  // Form State
  const [dateVal, setDateVal] = useState('');
  const [textVal, setTextVal] = useState(''); // Motivo or Estimated Time
  const [amountVal, setAmountVal] = useState(''); // Budget or Cost

  const openModal = (type: ModalType, item: MaintenanceItem) => {
    ...
    <div className="space-y-8">
      ...
      {ROOMS.map(room => (
          <div key={room.id} className="border border-gray-800 rounded p-4 bg-gray-900/50">
              <h2 className="text-xl font-bold mb-4" style={{ color: room.hex }}>Mantenimiento {room.name}</h2>
              <div className="grid gap-4">
                  ...
                  {items.filter(i => i.roomId === room.id && i.status === 'OK').length === 0 && (
                      <p className="text-gray-500 text-sm italic">Todos los items de esta sala están en reparación.</p>
                  )}
              </div>
          </div>
      ))}
